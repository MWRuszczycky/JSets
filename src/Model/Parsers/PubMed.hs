...
parsePMIDs :: Text -> Either T.ErrString [Text]
parsePMIDs json = P.parseJSON json >>= maybe err pure . readIDList
    where err = Left "Unable to read idlist from PMID request!"

readIDList :: P.JSON -> Maybe [Text]
readIDList obj = let  go (P.JStr t) = Just t
                      go _          = Nothing
                 in case P.objLookup [ "esearchresult", "idlist" ] obj of
                         Just (P.JArray ids) -> traverse go ids
                         _                   -> Nothing

parseCitations :: [Text] -> Text -> Either T.ErrString [T.Citation]
parseCitations ids json = pure []
